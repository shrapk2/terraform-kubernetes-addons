locals {
  ...
  #        repository                  = "https://k8s.scw.cloud/helm-charts"
       ...
       var.cert-manager["scaleway_webhook_dns"]
     )
    },
    var.cert-manager
  )
...
resource "kubernetes_namespace" "scaleway-webhook-dns" {
  count = local.cert-manager["scaleway_webhook_dns"]["enabled"] && local.cert-manager["scaleway_webhook_dns"]["install_cluster_wide"] == false ? 0 : 1
  ...
  metadata {
    ...
    labels = {
      name = local.cert-manager["scaleway_webhook_dns"]["namespace"]
    }
...
resource "helm_release" "scaleway-webhook-dns" {
  count                 = local.cert-manager["scaleway_webhook_dns"]["enabled"] ? 1 : 0
  repository            = local.cert-manager["scaleway_webhook_dns"]["repository"]
  name                  = local.cert-manager["scaleway_webhook_dns"]["name"]
  chart                 = local.cert-manager["scaleway_webhook_dns"]["chart"]
  version               = local.cert-manager["scaleway_webhook_dns"]["chart_version"]
  timeout               = local.cert-manager["scaleway_webhook_dns"]["timeout"]
  force_update          = local.cert-manager["scaleway_webhook_dns"]["force_update"]
  recreate_pods         = local.cert-manager["scaleway_webhook_dns"]["recreate_pods"]
  wait                  = local.cert-manager["scaleway_webhook_dns"]["wait"]
  atomic                = local.cert-manager["scaleway_webhook_dns"]["atomic"]
  cleanup_on_fail       = local.cert-manager["scaleway_webhook_dns"]["cleanup_on_fail"]
  dependency_update     = local.cert-manager["scaleway_webhook_dns"]["dependency_update"]
  disable_crd_hooks     = local.cert-manager["scaleway_webhook_dns"]["disable_crd_hooks"]
  disable_webhooks      = local.cert-manager["scaleway_webhook_dns"]["disable_webhooks"]
  render_subchart_notes = local.cert-manager["scaleway_webhook_dns"]["render_subchart_notes"]
  replace               = local.cert-manager["scaleway_webhook_dns"]["replace"]
  reset_values          = local.cert-manager["scaleway_webhook_dns"]["reset_values"]
  reuse_values          = local.cert-manager["scaleway_webhook_dns"]["reuse_values"]
  skip_crds             = local.cert-manager["scaleway_webhook_dns"]["skip_crds"]
  verify                = local.cert-manager["scaleway_webhook_dns"]["verify"]
  values = []
  namespace = local.cert-manager["scaleway_webhook_dns"]["install_cluster_wide"] ? kubernetes_namespace.cert-manager.*.metadata.0.name[count.index] : local.cert-manager["scaleway_webhook_dns"]["namespace"]
...
resource "kubernetes_secret" "scaleway_credentials" {
  metadata {
    ...
    namespace = local.cert-manager["scaleway_webhook_dns"]["install_cluster_wide"] ? local.cert-manager["namespace"] : local.cert-manager["scaleway_webhook_dns"]["namespace"]
  }
  data = {
...
data "kubectl_path_documents" "cert-manager_issuers" {
  pattern = "${path.module}/templates/cert-manager-issuers.yaml"
  vars = {
    acme_email    = local.cert-manager["acme_email"]
    namespace     = local.cert-manager["scaleway_webhook_dns"]["namespace"]
  }
}
...
resource "kubectl_manifest" "cert-manager_cluster_issuers" {
  count     = local.cert-manager["scaleway_webhook_dns"]["enabled"] && local.cert-manager["scaleway_webhook_dns"]["install_cluster_wide"] && local.cert-manager["enable_default_cluster_issuers"] ? length(data.kubectl_path_documents.cert-manager_cluster_issuers.documents) : 0
...
resource "kubectl_manifest" "cert-manager_issuers" {
  count     = local.cert-manager["scaleway_webhook_dns"]["enabled"] && local.cert-manager["scaleway_webhook_dns"]["install_cluster_wide"] == "false" && local.cert-manager["enable_default_cluster_issuers"] ? length(data.kubectl_path_documents.cert-manager_cluster_issuers.documents) : 0
  yaml_body = element(data.kubectl_path_documents.cert-manager_issuers.documents, count.index)
  depends_on = [
    helm_release.cert-manager,
    time_sleep.cert-manager_sleep
  ]
